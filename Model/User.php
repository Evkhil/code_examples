...
class Model_User
{
    ...
    public static function getById($userId)
    {
        ...
        if($userData) {
            ...
            throw new Exception('User not found', Model_Error::NOT_FOUND);
        }
    }
    ...
    public function register($params)
    {
        ...
        if(!empty($resIfExists)) {
            throw new Exception('Such account is already exists.', Model_Error::ALREADY_EXIST);
        }
        else {
            $resCreate = $tableUser->create($params);

            if(!$resCreate) {
                throw new Exception('Can\'t create new user. Try later.', Model_Error:: ERROR_CREATE_USER);
            }
            return $resCreate;
        }
    }
